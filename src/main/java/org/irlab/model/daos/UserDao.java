...
public class UserDao {
  ...
  public static Optional<User> findByName(EntityManager em, String name) {
    // Devuelve un optional (puede contener un User o nada).
    TypedQuery<User> q = em.createQuery("SELECT u FROM User u WHERE u.name = :name", User.class);
    // Query
    q.setParameter("name", name);
    // set parámetro nombre al que me pasaron.
    ...
    if (queryResult.size() > 1) {
      ...
      // Si no está vacía, devuelvo un optional con la lista.
    } else {
      return Optional.empty();
      // Devuelvo optional vacío.
    }
  }
}
