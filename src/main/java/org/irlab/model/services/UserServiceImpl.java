...
public class UserServiceImpl implements UserService {
    ...
    // Constructor comprueba que la BD esté creada.
    ...
    public String greet(@Nonnull String name) {
        ...
        try (var em = AppEntityManagerFactory.getInstance().createEntityManager()) {
            ...
            String greeting = maybeUser.map(User::getGreeting).orElse(DEFAULT_GREETING);
    ...
    public void setUserGreeting(@Nonnull String name, @Nonnull String greeting)
        ...
        try (var em = AppEntityManagerFactory.getInstance().createEntityManager()) {
            // Busco al usuario por orden. Me devuelve un optional.
            /* CAMBIO PROFE:
            ...
            */
            ...
            if (maybeUser.isEmpty()) {
                throw new UserNotFoundException(String.format("with name %s", name));
            }

            User user = maybeUser.get(); // El get si lo hago sobre uno vacío lanza expeción, pero eso ya lo comprobamos en el if de antes.
            ...
            try {
                ...
                em.getTransaction().rollback(); // Ante errores rollback y relanzo la excepción.
                throw e;
            }
        }
